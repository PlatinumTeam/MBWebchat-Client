 ...
 *
 * Slightly modified by HiGuy to use jQuery() instead of $() because the dang
 * libraries can't keep their functions to themselves.
 *
 * OPTIONS
 * -----------------------------------------------------------------------------
 *
 ...
 * -----------------------------------------------------------------------------
 *
 * 	var websocket = jQuery.gracefulWebSocket("ws://127.0.0.1:8080/");
 *
 * 	var websocket = jQuery.gracefulWebSocket({
 * 		fallbackPollParams:  {
		...
		gracefulWebSocket: function (url, options) {
			...
			function FallbackSocket() {
				...
				var fws = {
					...
					send: function (data) {
						...
						jQuery.ajax({
							...
							url: opts.fallbackSendURL + '?' + jQuery.param( getFallbackParams() ),
							data: data,
							dataType: 'text',
							...
							error: function (xhr) {
								success = false;
								jQuery(fws).triggerHandler('error');
							}
						});
						return success;
					},
					close: function () {
						...
						jQuery(fws).triggerHandler('close');
					},
					onopen: function () {},
					onmessage: function () {},
					onerror: function () {},
				...
				function getFallbackParams() {
					...
					return jQuery.extend({"previousRequest": fws.previousRequest, "currentRequest": fws.currentRequest}, opts.fallbackPollParams);
				}
				...
				openTimout = setTimeout(function () {
					fws.readyState = OPEN;
					//fws.currentRequest = new Date().getTime();
					jQuery(fws).triggerHandler('open');
					poll();
			...
			var ws = window.WebSocket ? new WebSocket(url) : new FallbackSocket();
	 		jQuery(window).unload(function () { ws.close(); ws = null });
			return ws;
		}
	});
